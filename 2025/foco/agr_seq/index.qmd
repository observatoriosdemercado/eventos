...
# Correlações entre Importância e Satisfação
...
# =====================================================
# 1. Configuração dos pares importância/satisfação
# =====================================================

temas = {
    "Tecnologias hídricas (água)": (
        "importancia_tec_hid",
        "satisfacao_tec_hid"
    ),
    "Melhoramento / genética": (
        "importancia_melhoramento",
        "satisfacao_melhoramento"
    ),
    "Biossalinidade / uso de água salina": (
        "importancia_biossalina",
        "satisfacao_biossalina"
    ),
    "Caprinos e pequenos ruminantes": (
        "importancia_caprinos",
        "satisfacao_caprinos"
    ),
    ...
    "Conservação de solo e água / recuperação de áreas": (
        "importancia_conservaçao",
        "satisfacao_conservaçao"
    ),
    "Forragem / alimentação animal no Semiárido": (
        "importancia_forragem",
        "satisfacao_forragem"
    ),
    "Bovinocultura de leite": (
        "importancia_bov_leite",
        "satisfacao_bov_leite"
    ),
    "Microrganismos / bioinsumos": (
        "importancia_microrgan",
        "satisfacao_microrgan"
    ),
    "Bioenergia / aproveitamento energético": (
        "importancia_energetico",
        "satisfacao_energetico"
    ),
    "Agroecologia / produção sustentável": (
        "importancia_agroecol",
        "satisfacao_agroecol"
    ),
    "Emissões / baixo carbono": (
        "importancia_baixo_c",
        "satisfacao_baixo_c"
    ),
}
...
# =====================================================
# 2. Funções auxiliares para calcular os percentuais
# =====================================================

def tabela_importancia_por_col(df, col):
    """
    Retorna uma linha com percentuais de cada resposta de importância
    e métricas agregadas (alta_importancia, baixa_importancia).
    """
    # Contar frequências relativas (%)
    dist = (
        df[col]
        .value_counts(dropna=False, normalize=True) * 100
    )

    muito_imp = dist.get("Muito importante", 0.0)
    moderado_imp = dist.get("Moderadamente importante", 0.0)
    pouco_imp = dist.get("Pouco importante", 0.0)
    nao_sei_imp = dist.get("Não sei", 0.0)

    return {
        "col_importancia": col,
        "% Muito importante": round(muito_imp, 1),
        "% Moderadamente importante": round(moderado_imp, 1),
        "% Pouco importante": round(pouco_imp, 1),
        "% Não sei (importância)": round(nao_sei_imp, 1),
        # duas formas úteis de resumir:
        "alta_importância_somente_muito(%)": round(muito_imp, 1),
        "alta_importância_muito+moderado(%)": round(muito_imp + moderado_imp, 1),
        "baixa_importância_pouco(%)": round(pouco_imp, 1),
    }

def tabela_satisfacao_por_col(df, col):
    """
    Retorna uma linha com percentuais de cada resposta de satisfação
    e métricas agregadas (satisfeitos vs não satisfeitos).
    Pressupõe que a coluna já esteja codificada como números 1..5
    ou textos equivalentes que você já transformou em número.
    """
    # Vamos primeiro mapear para rótulos textuais estáveis
    mapa_sat = {
        ...
        # se existir string tipo "nao sei", mantemos como "Não sei"
        "nao sei": "Não sei",
        "não sei": "Não sei",
        "Não sei": "Não sei",
        None: "Não sei",
    ...
    col_temp = df[col].map(mapa_sat).fillna(df[col].astype(str))
    ...
    tsat = dist.get("Totalmente satisfeito", 0.0)
    sat = dist.get("Satisfeito", 0.0)
    neutro = dist.get("Nem satisfeito nem insatisfeito", 0.0)
    insat = dist.get("Insatisfeito", 0.0)
    tinsat = dist.get("Totalmente insatisfeito", 0.0)
    nsei = dist.get("Não sei", 0.0)

    satisfeitos_pct = tsat + sat
    nao_satisfeitos_pct = neutro + insat + tinsat

    return {
        "col_satisfacao": col,
        "% Totalmente satisfeito": round(tsat, 1),
        "% Satisfeito": round(sat, 1),
        "% Nem satisfeito nem insatisfeito": round(neutro, 1),
        "% Insatisfeito": round(insat, 1),
        "% Totalmente insatisfeito": round(tinsat, 1),
        "% Não sei (satisfação)": round(nsei, 1),
        "satisfeitos_(4ou5)(%)": round(satisfeitos_pct, 1),
        "nao_satisfeitos_(1a3)(%)": round(nao_satisfeitos_pct, 1),
    }

# =====================================================
# 3. Construir as tabelas separadas
# =====================================================

linhas_importancia = []
linhas_satisfacao = []

for nome_tema, (col_imp, col_sat) in temas.items():
    imp_info = tabela_importancia_por_col(df, col_imp)
    sat_info = tabela_satisfacao_por_col(df, col_sat)

    imp_info["Tema"] = nome_tema
    sat_info["Tema"] = nome_tema

    linhas_importancia.append(imp_info)
    linhas_satisfacao.append(sat_info)

tabela_importancia = pd.DataFrame(linhas_importancia)
tabela_satisfacao = pd.DataFrame(linhas_satisfacao)

# =====================================================
# 4. Juntar tudo num diagnóstico único por tema
# =====================================================

diagnostico_temas = pd.merge(
    tabela_importancia,
    tabela_satisfacao,
    on="Tema",
    how="outer"
...
# Ordenar colunas em uma ordem mais amigável
cols_ordem = [
    "Tema",
    ...
    "% Pouco importante",
    "% Não sei (importância)",
    "alta_importância_somente_muito(%)",
    "alta_importância_muito+moderado(%)",
    "baixa_importância_pouco(%)",
    ...
    "% Não sei (satisfação)",
    "satisfeitos_(4ou5)(%)",
    "nao_satisfeitos_(1a3)(%)",
]

diagnostico_temas = diagnostico_temas[cols_ordem]

print("==== TABELA DE IMPORTÂNCIA POR TEMA ====")
print(tabela_importancia.sort_values("Tema"))

print("\n==== TABELA DE SATISFAÇÃO POR TEMA ====")
print(tabela_satisfacao.sort_values("Tema"))

print("\n==== DIAGNÓSTICO RESUMIDO (IMPORTÂNCIA x SATISFAÇÃO) ====")
print(diagnostico_temas.sort_values("Tema"))
```
...
# ============================================================
# 1. Definir os thresholds de corte
#    - Importância alta: >= 80% (muito + moderado)
#    - Satisfação alta: >= 55% (satisfeito + totalmente satisfeito)
# ============================================================

thr_importancia = 80.0
thr_satisfacao = 50.0

df_quad = diagnostico_temas.copy()

# Renomear para nomes curtos (só pra ficar mais legível)
df_quad = df_quad.rename(columns={
    "alta_importância_muito+moderado(%)": "importancia_alta_pct",
    "satisfeitos_(4ou5)(%)": "satisfacao_alta_pct"
})

# ============================================================
# 2. Criar flags (True/False) para alta importância e alta satisfação
# ============================================================

df_quad["flag_importancia_alta"] = df_quad["importancia_alta_pct"] >= thr_importancia
df_quad["flag_satisfacao_alta"] = df_quad["satisfacao_alta_pct"] >= thr_satisfacao

# ============================================================
# 3. Função para classificar o quadrante estratégico
# ============================================================

def classificar_quadrante(row):
    imp = row["flag_importancia_alta"]
    sat = row["flag_satisfacao_alta"]

    if imp and sat:
        return "FORÇA INSTITUCIONAL (Alta importância & Alta satisfação)"

    if imp and not sat:
        return "GARGALO CRÍTICO / PRIORIDADE (Alta importância & Baixa satisfação)"

    if (not imp) and sat:
        return "MANUTENÇÃO / ATENDIMENTO ADEQUADO (Importância média & Satisfação alta)"
    
    # (not imp) and (not sat)
    return "BAIXA PRIORIDADE IMEDIATA (Importância média & Satisfação baixa)"

df_quad["Quadrante"] = df_quad.apply(classificar_quadrante, axis=1)

# ============================================================
# 4. Selecionar só as colunas mais importantes para mostrar
# ============================================================

classificacao_final = df_quad[[
    "Tema",
    "importancia_alta_pct",
    "satisfacao_alta_pct",
    "flag_importancia_alta",
    "flag_satisfacao_alta",
    "Quadrante"
]].copy()

# Deixar os nomes bonitos pro relatório
classificacao_final = classificacao_final.rename(columns={
    "importancia_alta_pct": "% Importância (Muito+Moderado)",
    "satisfacao_alta_pct": "% Satisfeitos (4 ou 5)",
    "flag_importancia_alta": "Importância alta?",
    "flag_satisfacao_alta": "Satisfação alta?",
})

# ============================================================
# 5. Criar tabela agregada por quadrante
#    - Agrupa os temas por Quadrante e junta numa linha só
# ============================================================
...
# Ordenar os quadrantes em uma ordem lógica para apresentação
ordem_quadrantes = [
    "FORÇA INSTITUCIONAL (Alta importância & Alta satisfação)",
    "GARGALO CRÍTICO / PRIORIDADE (Alta importância & Baixa satisfação)",
    "MANUTENÇÃO / ATENDIMENTO ADEQUADO (Importância média & Satisfação alta)",
    "BAIXA PRIORIDADE IMEDIATA (Importância média & Satisfação baixa)",
]

resumo_quadrantes["__ordem__"] = resumo_quadrantes["Quadrante"].apply(
    lambda q: ordem_quadrantes.index(q) if q in ordem_quadrantes else 999
)
resumo_quadrantes = resumo_quadrantes.sort_values("__ordem__").drop(columns="__ordem__")

# ============================================================
# 6. Mostrar resultados
# ============================================================

print("==== CLASSIFICAÇÃO POR TEMA ====")
print(classificacao_final.sort_values("Quadrante"))
...
```{python}
resumo_quadrantes_relatorio = resumo_quadrantes.copy()

# renomear para ficar mais curto dentro do slide
resumo_quadrantes_relatorio["Quadrante"] = resumo_quadrantes_relatorio["Quadrante"].replace({
    "FORÇA INSTITUCIONAL (Alta importância & Alta satisfação)": "FORÇA INSTITUCIONAL",
    "GARGALO CRÍTICO / PRIORIDADE (Alta importância & Baixa satisfação)": "GARGALO CRÍTICO / PRIORIDADE",
    "MANUTENÇÃO / ATENDIMENTO ADEQUADO (Importância média & Satisfação alta)": "MANUTENÇÃO / ATENDIMENTO ADEQUADO",
    "BAIXA PRIORIDADE IMEDIATA (Importância média & Satisfação baixa)": "BAIXA PRIORIDADE IMEDIATA",
})

print(resumo_quadrantes_relatorio.to_markdown(index=False))
```
